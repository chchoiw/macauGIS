          ...
          if (displayMask(x, y,imageData,width)) {
            ...
            if (coord) 
            {
              ...
              var colorLevel=params.colorScale;
                ...
                {
                  if (zScale>= colorLevel[i][0]&& zScale< colorLevel[i+1][0])
                  { 
    ...
    function drawLegend(bounds)
    {
            ...
            var lastVarName = ctxindex.getAttribute("varName");             
              ...
              if (isCanvasBlank(ctxindex) || lastVarName!=dataKey)
              {
                ...
                var leng=colorLevel.length;
                var n = Math.floor(bounds[1][1]/leng);
                ...
                for (var i=0; i<leng-1;i++)
                {
                  ...
                  g.fillRect(0,i*(n) , 50, (i+1)*(n));
                  g.font = "bolder 15px Arial";
                  g.fillStyle="#fff";
                  g.fillText((colorLevel[i][0]).toFixed(0),20,(i+0.5)*(n));
                // console.log(Math.round((min+range/leng*i),2).toString()+'m/s')
                // g.fillText('m/s',i*(n),30);
                }
                g.fillStyle="#fff";
                g.fillText(params.displayOptions.speedUnit,20,(leng-0.5)*(n));
              }
    };
    var windy = {
      params: params,
