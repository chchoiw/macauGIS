...
var Windy = function Windy(params) {
	...
	var createBuilder = function createBuilder(data) {
		...
		data.forEach(function (record) {
			// console.log(record.header,record.header.parameterCategory + "," + record.header.parameterNumber)
	...
	var buildGrid = function buildGrid(data, callback) {
		...
		// 	"ny":180,
		// }	
		λ0 = longlatdict["lo1"] //header.lo1;
		// console.log("header.lo1", header.lo1);
		φ0 =longlatdict["la1"] //header.la1; // the grid's origin (e.g., 0.0E, 90.0N)

		// Δλ = header.dx;
		// delta long
		Δλ =( longlatdict["lo2"]-longlatdict["lo1"])/ longlatdict["nx"]
		// distance between grid points (e.g., 2.5 deg lon, 2.5 deg lat)
		Δφ = ( longlatdict["la1"]-longlatdict["la2"])/ longlatdict["ny"]
		//0.0005895;//header.dy; 
		// console.log(Δφ,header.dx)
		// λ0=100
		// φ0=30
		// Δλ=0.139;
		// Δφ =0.11;
		ni = longlatdict["nx"]//header.nx;
		nj = longlatdict["ny"]//header.ny; // number of grid points W-E and N-S (e.g., 144 x 73)
  ...
  */
	var interpolate = function interpolate(λ, φ) {
		...
		var i = (λ - λ0)/ Δλ; //floorMod(λ - λ0, 360) / Δλ; // calculate longitude index in wrapped range [0, 360)
		// console.log("in interpolate", λ,λ0,Δλ,i)
		var j = (φ0 - φ) / Δφ; // calculate latitude index in direction +90 to -90
		// console.log(Δφ)
		// console.log("in interpolate 290",λ,φ,λ0,φ0)
		var fi = Math.floor(i),
		    ci = fi + 1;
		var fj = Math.floor(j),
		    cj = fj + 1;
		...
		if (row = grid[fj]) {
			...
			if (isValue(g00) && isValue(g10) && (row = grid[cj])) {
				...
				if (isValue(g01) && isValue(g11)) {
					// console.log("enter",g01,g11)
					// All four points found, so interpolate the value.
					// this is bilinearinterpolate
					//[u, v, tmp];
					var aa=builder.interpolate(i - fi, j - fj, g00, g10, g01, g11);
					// console.log("inter check why wind is null",aa)
					return aa;
				}
			}
		}
  ...
  */
   ...
   */
	...
	var interpolateField = function interpolateField(grid, bounds, extent, callback) {
		...
		var columns = [];
		var x = bounds.x;
		// here bounds is px of div map
		// bounds={height 800 width: 500 x: 0 xMax: 500 y: 0y Max: 799}
		// extend
		// {"south": 0.3856697890030696,"north": 0.38788957505761457,"east": 1.98262223144257,"west": 1.9811242033294005,"width": 500,"height": 800}
			...
			for (var y = bounds.y; y <= bounds.yMax; y += 2) {
				...
				if (coord) {
					...
					if (isFinite(λ)) {
						...
						//  wind =[-5.284615562221136, -0.08998432735875328, 297.1045347781697]
						// u ,v , temp

						if (wind) {
							var a=wind[0];
							var b=wind[1];
							var speed=Math.sqrt((a * a) + (b * b))
							// velocityScale = speed*1./100;
							// console.log(projection, λ, φ, x, y, velocityScale, wind,speed, extent)
...
(function (root, factory) {
   ...
   */
		...
		_loadLocalData: function _loadLocalData() {
			...
			$.getJSON('demo2.json', function (data) {
